...
label_colors[-1] = (255, 255, 255)  # Edge
label_colors[-2] = (128, 128, 128)  # In queue

def np2img(nparr, filename, folder='result_img'):
...
def img2grayscale(img):
    gray_arr = 0.11 * img[:, :, 0] + 0.59 * img[:, :, 1] + 0.3 * img[:, :, 2]
...
def mouse_marked(img, labels=9):
    ...
    def mouse_callback(event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN or (event == cv2.EVENT_MOUSEMOVE and flags & cv2.EVENT_FLAG_LBUTTON):
            ...
            cv2.circle(label_map, (y, x), 5, current_label, -1)
...
def watershedtoimg(label_img, original_img):
    h, w = label_img.shape[:2]
    seg_img = np.zeros((h, w, 3), dtype=np.uint8)
    for label, color in label_colors.items():
        seg_img[label_img == label] = color

    # Overlay boundaries on original image
    edge_overlay = original_img.copy()
    edge_overlay[label_img == -1] = (255, 255, 255)  # or change to any edge color
    return seg_img, edge_overlay
...
def main():
    ...
    for i in range(len(input_file)):
        ...
        img = getimg(img_str)
        label_map, mark_img = mouse_marked(img)
        mask_img = label2maskimg(mark_img, img)
        watershedarr = watershedsegment(img, label_map)
        watershedimg, overlay_img = watershedtoimg(watershedarr, img)

        showimg("Segment", overlay_img)
        ...
        np2img(overlay_img, input_file[i] + "_seg_overlay" + output_format)
